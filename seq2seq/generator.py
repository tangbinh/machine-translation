...
class SequenceGenerator(object):
    ...
    def generate(self, src_tokens, src_lengths):
        ...
        with torch.no_grad():
            return self._generate(src_tokens, src_lengths)

    def _generate(self, src_tokens, src_lengths):
        ...
        for step in range(maxlen + 1):  # Extra step for EOS marker
            ...
            lprobs, avg_attn_scores = self._decode(tokens[:, :step + 1], encoder_outs, incremental_states, log_probs=True)
    ...
    def _decode(self, tokens, encoder_out, incremental_states, log_probs):
        with torch.no_grad():
